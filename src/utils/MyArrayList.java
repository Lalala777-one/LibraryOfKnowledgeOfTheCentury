...
// Iterable (интерфейс) означает что все объекты этого класса можно перебирать (итерировать). Содержит всего 1 метод,
// который возвращает итератор (тот самый объект-закладку, который и позволяет перебирать коллекцию)
// Iterable <T> iterator() - возвращает итератор для коллекции


// Iterator - собственно итератор, который позволяет обходить коллекцию. Имеет 3 метод, из них два обязательных
...
public class MyArrayList<T> implements MyList<T>{
    ...
    public MyArrayList(T[] array) {
        if (array == null || array.length == 0) {
            this.array = (T[]) new Object[10]; // ToDo нельзя создать объект типа T в моменте написания программы по этому указавается Object и приводится к T
            // проверка идет на этапе компеляции
        } else {
...
//           Индекс не валидный
    ...
    public boolean contains(T value) {
        // переиспользовали метод  indexOf
    ...
    public boolean isEmpty() {
        return cursor == 0;  // если курсов равен 0 то значит у нас нет элементов во внутреннем массиве
    }

    @Override
    public void set(int index, T value) {
        if (index >= 0 && index < cursor) {
            // если индекс "правильный" присваиваем новое значение, если нет - ничего не делаем
            array[index] = value;
        }
    }


    // метод должен вернуть наш магическим массив в виде обычного массива

    @Override
    public T[] toArray() {
                /*
                Создать новый массив разметностью coursor
                Пройтись по нашему внутреннему массиву и скопировать все элементы в новый массив
                Вернуть ссылку
                 */
        //  Взять какой-то объект из моего массива и узнать на стадии ВИПОЛНЕНИЯ ПРОГРАММЫ тип этого массива
        // И потом могу создать массив этого типа
        ...
        // Создаю массив того же типа как и нулевой элемент
    ...
    // переопределить метод из интерфейса Iterator
    @Override
    public Iterator<T> iterator() {
        // Todo - написать имплементацию итератора !!
    ...
    private class MyIterator implements Iterator<T> {
        ...
        public boolean hasNext() {
            ...
            return currentIndex < cursor; // переменная из класса MyArrayList ;
        }
        ...
        public T next() {
            ...
            // return array[currentIndex++]; аналог записи
        }
    } // клас MyIterator внутри класса MyArrayList
